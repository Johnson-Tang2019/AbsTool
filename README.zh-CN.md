...
按 R+B 后点击顶部“服务器统计”，Fabric 和 NeoForge 均直接在该标签内展示统计，
顶部导航条保持不变，无启用开关，也无需再次点击打开。
概览、在线时间、方块放置、死亡和趋势数据需要服务器提供兼容的 AbsServerTool 协议 v2 接口；
不支持该接口的服务器会显示明确状态，而非空白页面。

## 低顶空鞘翅快速起飞

在 R+B → 优化 → 鞘翅辅助中开启“低顶空快速起飞”。默认使用潜行 + 跳跃，也可选择独立快捷键（默认 G）。
普通物品栏中有可用鞘翅即可启动：先换装，再跳跃，待原版同步真实离地状态后发送一次起飞请求，由服务器决定是否进入滑翔。不提供推进。
Fabric 可在 Tweakeroo 0.29.5 自动切换鞘翅开启时委托它换装；未安装或未开启时，Fabric／NeoForge 使用 AbsTool 的一次正常库存交换。
不增加必需依赖；未知且开启自动换装的 Tweakeroo 版本会安全取消。AbsTool 不在落地时强制恢复胸甲。
详见[实现与测试记录](docs/ELYTRA_LAUNCH.md)。

此功能仍为实验性功能，默认关闭。已验证服务器确认起飞；独立的操纵钻出出口测试超时，尚不保证能够顺利飞出空间。
...
“优化”中提供实验性的 **Fabric** Litematica／Iris 适配。AUTO 适用于 Minecraft 26.2 和 OpenGL，支持以下已核查版本组合：

| Litematica | 可搭配的 MaLiLib |
| --- | --- |
| 0.28.3／0.28.4 | 0.29.2～0.29.6 |
| 0.28.5 | 0.29.4～0.29.6 |
| 0.28.6／0.28.8 | 0.29.5／0.29.6 |

Iris／Sodium 选择一组：**1.11.1／0.9.0**、**1.11.2／0.9.1** 或 **1.11.4／0.9.2**，均须为 Minecraft 26.2 版本。
依赖需要分别安装；未核查版本或被修改的适配目标类仍会禁用适配。OFF 恢复上游绘制。
NeoForge 暂不启用这项新适配。
...
快捷设置增加 **混合潜影盒显示最多物品**（默认开启）：把省略号替换为同类物品总数量最多的图标，
跨槽位累加，同种物品的不同组件也合并计数；并列时取最靠前槽位的物品。
需要开启 TweakerMore 的“显示内容图标”。自定义名称指定图标、混合物品装填条仍遵守原设置。
关闭该选项即可恢复省略号。此渲染扩展要求已核查的 TweakerMore 3.33.2 渲染代码，普通快捷设置独立保留。
